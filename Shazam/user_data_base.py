...
db_user = pw.SqliteDatabase(data_base_path)
...
def create_user(user_name: str, user_password: str):
    ...
    user0 = Users(username=user_name, password=user_password)
    return user0


def add_user(data_base_path: str, user_name: str, user_password: str):
    ...
    user0 = create_user(user_name, user_password)
    user0.save()
    data_base.close()
...
if __name__ == '__main__':
    ...
    add_user(data_base_path, username1, password1)
    add_user(data_base_path, username2, password2)
    add_user(data_base_path, username3, password3)


    # verify the information of connection
    assert_connection(username1, password1, data_base_path)
    assert_connection(username2, password2, data_base_path)
    assert_connection(username3, password3, data_base_path)

    #Add musics of users 1,2,3
    add_user_music(data_base_path, username1, user1_music1_title, user1_music1_author)
    add_user_music(data_base_path, username3, user3_music1_title, user3_music1_author)
    add_user_music(data_base_path, username2, user2_music1_title, user2_music1_author)
    add_user_music(data_base_path, username3, user3_music2_title, user3_music2_author)
    add_user_music(data_base_path, username1, user1_music2_title, user1_music2_author)
    add_user_music(data_base_path, username2, user2_music2_title, user2_music2_author)

    #See users musics
    see_user_musics(data_base_path, username1)
    print('-----------------------------------')
    see_user_musics(data_base_path, username2)
    print('-----------------------------------')
    see_user_musics(data_base_path, username3)
    print('-----------------------------------')
